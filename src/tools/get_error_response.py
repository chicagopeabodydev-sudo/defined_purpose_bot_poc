...
_ERROR_PATH = os.path.join(os.path.dirname(__file__), "..", "resources", "error_messages.json")
...
def get_error_response(error_type: str) -> str:
    ...
    error_type should be one of: 'off-topic', 'sexual-content', 'sarcastic-response'.
    ...
    if not matches:
        matches = [m for m in messages if m["errorType"] == "off-topic"]
